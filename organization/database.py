...
class Database:
    ...
    async def get_all_groups(self) -> list:
        """
        TASK 3.2: Возвращает уникальные group_name из таблицы users.
        Используется в config.get_available_groups() для мёржа с config.yaml.
        """
        async with self.conn.cursor() as cursor:
            await cursor.execute(
                "SELECT DISTINCT group_name FROM users WHERE group_name IS NOT NULL AND group_name != \'\'"
            )
            rows = await cursor.fetchall()
